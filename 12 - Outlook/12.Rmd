...
bundestag_elections %>%
  filter(year==2021) -> bundestag_election_2021
parliament_data(
  election_data = bundestag_election_2021,
  type = "semicircle",
  parl_rows = 15,
  party_seats = bundestag_election_2021$seats) -> bundestag_2021
ggplot(bundestag_2021, aes(x, y, colour=party_short)) +
  geom_parliament_seats(size=3) + 
  geom_parliament_bar(colour = colour, party = party_long) +
  #set theme_ggparliament
  theme_ggparliament() +
  ...
  labs(colour = NULL, 
       title = "Bundestagswahl 2013") +
  scale_colour_manual(values = bundestag_2021$colour, 
                      limits = bundestag_2021$party_short)
```
